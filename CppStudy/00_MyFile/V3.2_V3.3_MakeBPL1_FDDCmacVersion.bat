@REM 完成增量编译并且自动制作出生成V3.2 V3.3的BPL1 FDD bpl1.lws和bpl1.so，放到当前目录的的一个文件夹内
...
@set CMAC_VERSION_PATH=%THIS_PATH%BPL1_FDD_CmacVersion%date_time4%
...
@for /f %%i in (temp2.txt) do (
  @set /a count = !count! + 1 )
@if not 2 equ !count! ( @cls & @echo 编译失败 & @goto End )
...
copy /Y REL_P4080-core_CMAC*.* %THIS_PATH%eNodeB\eBBU\BPL1\RMI_CC16\FDD 

del /Q %THIS_PATH%eNodeB\Version_SW\LTE_FDD\*.*
cd %THIS_PATH%eNodeB\version_create\script\swbat\lte_fdd
echo %CMAC_VERSION_PATH%
call LTE_FDD_BPL1_LWS.bat
cd %THIS_PATH%eNodeB\version_create\script\swbat\lte_fdd
echo %CMAC_VERSION_PATH%
call LTE_FDD_BPL1_SO.bat
echo %CMAC_VERSION_PATH%
cd %CMAC_VERSION_PATH%
copy /Y %THIS_PATH%eNodeB\Version_SW\LTE_FDD\*.LfBpl1Rmios .
:End
@pause
